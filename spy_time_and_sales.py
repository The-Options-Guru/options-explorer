"""
SPY Time and Sales Data Collector
===================================
Collects 0DTE SPY options Time and Sales data via dxFeed demo API.

Requirements:
    pip install websocket-client

Data Source:
    - dxFeed demo endpoint (15-minute delayed data)
    - No authentication required

Output:
    - CSV files saved to 'historical data/' directory
    - Saves every 60 seconds
    - File naming: spy_tas_data_YYYY-MM-DD.csv
    
CSV Columns:
    - date, time: Trade timestamp
    - symbol: Option symbol (e.g., .SPY241218C580)
    - event_type: TIME_AND_SALE or SPY_QUOTE
    - price, size: Trade details
    - bid_price, ask_price: Quotes at time of trade
    - time_and_sale_flags: Trade flags (ETH, SPREAD, ISO, etc.)
    - exchange, sequence, exchange_sale_conditions, aggressor_side
    - spy_price, spy_bid, spy_ask: SPY quote at time of trade

Usage:
    python spy_tas_collector.py
    
    Press Ctrl+C or send SIGTERM to stop gracefully
    
Strike Range:
    - Dynamically calculates 15% OTM strikes around current SPY price
    - 1-point increments
...
print("=" * 60)
print("SPY Time and Sales Collector - Starting")
print("=" * 60)
sys.stdout.flush()
...
def generate_spy_options(center_price, otm_percentage=0.15, increment=1, expiration=None):
    """Generate SPY options symbols based on percentage OTM
    
    Args:
        center_price: Current SPY price
        otm_percentage: How far OTM to go (0.15 = 15%)
        increment: Strike spacing (1 for SPY)
    ...
    for strike in strikes:
        options.append(f".SPY{expiration}C{strike}")
        options.append(f".SPY{expiration}P{strike}")

    print(f"\n[STRIKES] Strike Range Calculation:")
    print(f"          SPY Price: {center_price}")
...
def save_data_to_csv(time_and_sales_buffer, spy_quotes_buffer):
    """Save TimeAndSale events and SPY quotes to CSV file."""
    global last_successful_save

    if not time_and_sales_buffer and not spy_quotes_buffer:
        return False

    # Ensure the historical data directory exists
    os.makedirs("historical data", exist_ok=True)

    filename = os.path.join("historical data", f"spy_tas_data_{datetime.now().strftime('%Y-%m-%d')}.csv")
    ...
    try:
        ...
        with open(filename, "a", newline="", encoding="utf-8") as f:
            ...
            # Write SPY quotes
            if spy_quotes_buffer:
                writer.writerows(spy_quotes_buffer)
                print(f"[SAVE] Saved {len(spy_quotes_buffer)} SPY quotes -> {filename}")
                spy_quotes_buffer.clear()
        ...
        total_items = len(time_and_sales_buffer) + len(spy_quotes_buffer)
...
def main():
    ...
    # Get SPY midpoint
    print("\n[DATA] Fetching SPY midpoint from demo feed...")
    ws.send(json.dumps({
        "type": "FEED_SUBSCRIPTION",
        "channel": 1,
        "add": [{"symbol": "SPY", "type": "Quote"}]
    }))
        ...
        try:
            ...
            if msg.get("type") == "FEED_DATA":
                for data in msg.get("data", []):
                    if data.get("eventSymbol") == "SPY" and data.get("eventType") == "Quote":
                        bid = data.get("bidPrice")
                        ask = data.get("askPrice")
                        if bid and ask:
                            spy_price = round((bid + ask) / 2)
                            print(f"[OK] SPY midpoint = {spy_price}")
                            break
            if spy_price:
                break
        except Exception as e:
    ...
    if not spy_price:
        spy_price = 580
        print("[WARN] Using default SPY price: 580")

    # Generate SPY options with today's expiration - 15% OTM each direction
    expiration = get_todays_expiration()
    print(f"[DATE] Using today's expiration: {expiration}")
    options = generate_spy_options(spy_price, otm_percentage=0.15, increment=1, expiration=expiration)
    print(f"[INFO] Sample symbols: {options[:4]}")
    print(f"       Lowest strike: {options[0]}")
    print(f"       Highest strike: {options[-1]}")

    # Subscribe to TimeAndSale ONLY + SPY Quote
    ...
    # Add SPY Quote subscription
    tas_events.append({"symbol": "SPY", "type": "Quote"})
    ...
    print(f"[OK] Subscribed to {len(tas_events)} events (TAS + SPY Quote)")
    ...
    try:
        while not shutdown_requested:
                    ...
                    for data in msg.get("data", []):
                        ...
                        if etype == "TimeAndSale":
                            ...
                            time_and_sales_buffer.append({
                                ...
                                "spy_price": latest_spy_quote.get("midpoint", "N/A"),
                                "spy_bid": latest_spy_quote.get("bid", "N/A"),
                                "spy_ask": latest_spy_quote.get("ask", "N/A"),
                            })
            ...
            if time.time() - last_stats_time >= 30:
                ...
                print(f"        SPY Quote: {stats['Quote']:,}")
                print(f"        Other: {stats['other']:,}")
                print(f"        TAS Buffer: {len(time_and_sales_buffer):,} events")
                print(f"        SPY: {latest_spy_quote.get('midpoint', 'N/A')}")
                
                stats = {k: 0 for k in stats}
                last_stats_time = time.time()

            # Save SPY quote every minute
            if time.time() - last_spy_quote_time >= 60:
                if latest_spy_quote:
                    ...
                    spy_quotes_buffer.append({
                        "date": date_str,
                        "time": time_str,
                        "symbol": "SPY",
                        "event_type": "SPY_QUOTE",
                        "price": latest_spy_quote.get("midpoint", "N/A"),
                        "size": "N/A",
                        "exchange": "QUOTE",
                        "bid_price": latest_spy_quote.get("bid", "N/A"),
                        "ask_price": latest_spy_quote.get("ask", "N/A"),
            ...
            if time.time() - last_save_time >= 60:
                save_data_to_csv(time_and_sales_buffer, spy_quotes_buffer)
                last_save_time = time.time()
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("SPY Time and Sales Collector")
